...
class Web4pro_Fastorder_Block_Adminhtml_Orders_View_Totals extends Mage_Adminhtml_Block_Sales_Totals //Mage_Adminhtml_Block_Sales_Order_Abstract
{
    ...
    protected function _initTotals()
    {
        ...
        if ($magentoOrder) {
            $discount = $magentoOrder->getDiscountAmount();
            if(!empty($discount)){
                $this->_totals['discount'] = new Varien_Object(
                    array(
                        'code' => 'discount',
                        'strong' => true,
                        'value' => $magentoOrder->getDiscountAmount(),
                        'base_value'=> $magentoOrder->getBaseDiscountAmount(),
                        'label' => $this->helper('web4pro_fastorder')->__('Discount'). ": ".$magentoOrder->getDiscountDescription(),
                        'area' => 'footer'
                    )
                );
            }
